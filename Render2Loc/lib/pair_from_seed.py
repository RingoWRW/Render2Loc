...
from .transform import  parse_pose_list, parse_db_intrinsic_list, parse_image_lists
...
def get_pairs_imagepath(pairs, render_path, image_path):
    ...
    for query_name, imgr_name_list in pairs.items():
        ...
        for imgr_name in imgr_name_list:
            ...
            imgr_pth = render_path / 'rgbs'/imgr
            
            exrr_pth = render_path / 'depths'/(imgr.split('.')[0] + '0001.exr') #!  
            render = [imgr_pth, exrr_pth] 
            renders.append(render)
        render_dir[imgq_pth] = renders
    return render_dir
...
def main(
    ...
    render_camera = Path(render_camera)
    query_camera = Path(query_camera)
